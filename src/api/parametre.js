...
export const getParametres = async (page = 1, query = '') => {
    const response = await $fetch(`${import.meta.env.VITE_API_URL}/api/parametres?page=${page}${query}`, {
        method:'get',
        headers: {
            ...defaultHeaders,
...
export const getParametresByZone = async (id) => {
    const response = await $fetch(`${import.meta.env.VITE_API_URL}/api/configuration/zone/${id}`, {
        method:'get',
        headers: {
            ...defaultHeaders,
            'Content-Type': 'application/ld+json',
