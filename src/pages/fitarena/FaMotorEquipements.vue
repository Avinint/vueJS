...
onMounted(async () => {
  equipements.value = await getEquipements(
    fitArenaId.value,
    1,
      '&typeEquipement.categoryTypeEquipement.code=motorise&fitArena.id='+ fitArenaId.value
  )
  typeEquipements.value = setEquipementModes(
    await getTypeEquipements(
      1,
      '&categoryTypeEquipement.code=motorise&equipements.fitArena=' + fitArenaId.value
