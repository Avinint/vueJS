<template>
  ...
  <form @submit.prevent="deleteFitArenaValidation(deleteOrganismeId)">
    <ValidationModal
        v-if="delete_modal"
        type="delete"
  ...
  <form @submit.prevent="updatOrganismeValidation()">
...
const mapApiToData = (organisme) => {
  ...
  commentaire.value = organisme.commentaire
...
const updateOrganismeValidation = async () => {
  ...
  cancel()
  fit_arenas.value = await getOrganismes()
}
